...
func TestSendCoins(t *testing.T) {
	...
	_, err = wt.wallet.SendCoins(types.NewCurrency64(5000), types.UnlockConditionProxy{}, nil)
...
func TestIntegrationSendOverUnder(t *testing.T) {
	...
	_, err = wt.wallet.SendCoins(tooManyCoins, types.UnlockConditionProxy{}, nil)
	if err != modules.ErrLowBalance {
	...
	_, err = wt.wallet.SendCoins(reasonableCoins, types.UnlockConditionProxy{}, nil)
	if err != nil {
...
func TestIntegrationSpendHalfHalf(t *testing.T) {
	...
	_, err = wt.wallet.SendCoins(halfPlus, types.UnlockConditionProxy{}, nil)
	if err != nil {
		t.Error("unexpected error: ", err)
	}
	_, err = wt.wallet.SendCoins(halfPlus, types.UnlockConditionProxy{
		Condition: &types.UnlockHashCondition{TargetUnlockHash: types.NewUnlockHash(0, crypto.Hash{1})},
	}, nil)
...
func TestIntegrationSpendUnconfirmed(t *testing.T) {
	...
	_, err = wt.wallet.SendCoins(someMore, types.UnlockConditionProxy{
		Condition: &types.UnlockHashCondition{TargetUnlockHash: types.NewUnlockHash(0, crypto.Hash{1})},
	}, nil)
	if err != nil {
