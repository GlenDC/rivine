...
func (cmds *commands) rootCommand(*cobra.Command, []string) {
	...
	err = cmds.runDaemon()
	if err != nil {
		cli.DieWithError("failed to run daemon", err)
	}
}

func (cmds *commands) runDaemon() error {
	// Print a startup message.
	fmt.Println("Loading...")
	//loadStart := time.Now()

	//moduleIdentifiers := cmds.moduleSetFlag.ModuleIdentifiers().Identifiers()
	//compontentsToLoad := len(moduleIdentifiers) + 1 // also need to load server

	return errors.New("TODO... figure out how to do this cleanly")
}
